...
class InpaintCAModel(Model):
    ...
    def build_unet_discriminator(OUTPUT_CHANNELS):
        with tf.variable_scope('unet_discriminator', reuse=reuse):
    ...
    def build_unet_graph_with_losses(
            self, FLAGS, batch_data, training=True, summary=False,
            reuse=False):
        if FLAGS.guided:
            batch_data, edge = batch_data
        ...
        if FLAGS.guided:
            ...
            xin = batch_incomplete
        x1, x2, offset_flow = self.build_inpaint_net(
            xin, mask, reuse=reuse, training=False,
            padding=FLAGS.padding)
        batch_predicted = x2
        losses = {}
        # apply mask and complete image
        ...
        losses['ae_loss'] = FLAGS.l1_loss_alpha * tf.reduce_mean(tf.abs(batch_pos - x1))
        losses['ae_loss'] += FLAGS.l1_loss_alpha * tf.reduce_mean(tf.abs(batch_pos - x2))
        # UNET DISCRIMINATOR LOSS
        unet_masks = self.build_unet_discriminator(training=True, reuse=reuse)
        unet_loss = FLAGS.l1_loss_alpha * tf.reduce_mean(tf.abs(mask - unet_masks))
        losses['unet_loss'] = unet_loss
        if summary:
            scalar_summary('losses/unet_loss', losses['unet_loss'])
            viz_img = [batch_pos, batch_incomplete, batch_complete, unet_masks, mask]
            images_summary(
                tf.concat(viz_img, axis=2),
                'raw_images_complete', FLAGS.viz_max_out)
        # if summary:
        #     scalar_summary('losses/ae_loss', losses['ae_loss'])
        #     if FLAGS.guided:
        #         viz_img = [
        ...
        # # wgan with gradient penalty
        # if FLAGS.gan == 'sngan':
        #     pos_neg = self.build_gan_discriminator(batch_pos_neg, training=False, reuse=reuse)
        #     pos, neg = tf.split(pos_neg, 2)
        #     g_loss, d_loss = gan_hinge_loss(pos, neg)
        #     losses['g_loss'] = g_loss
        #     losses['d_loss'] = d_loss
        ...
        if summary:
            ...
            # # gradients_summary(losses['ae_loss'], x1, name='ae_loss_to_x1')
            # gradients_summary(losses['ae_loss'], x2, name='ae_loss_to_x2')
            gradients_summary(losses['unet_loss', losses['unet_loss']])
        # losses['g_loss'] = FLAGS.gan_loss_alpha * losses['g_loss']
        # if FLAGS.ae_loss:
        #     losses['g_loss'] += losses['ae_loss']
        g_vars = tf.get_collection(
        ...
        d_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, 'discriminator')
        d_u_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, 'unet_discriminator')
        return g_vars, d_vars, d_u_vars, losses
