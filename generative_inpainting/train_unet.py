...
if __name__ == "__main__":
    ...
    g_vars, d_vars, u_vars, losses = model.build_unet_graph_with_losses(FLAGS, images)
    # validation images
    if FLAGS.val:
    ...
    u_optimizer = tf.train.AdamOptimizer(lr, beta1=0.5, beta2=0.999)
    ...
    unet_training_callback = ng.callbacks.SecondaryMultiGPUTrainer(
        num_gpus=FLAGS.num_gpus_per_job,
        pstep=1,
        ...
        graph_def=multigpu_graph_def,
        graph_def_kwargs={
            'model': model, 'FLAGS': FLAGS, 'data': data, 'loss_type': 'u'},
    )
